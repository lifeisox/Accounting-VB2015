...
Public Class EducationCodeForm
    ...
    Public Sub New()
        ...
        _SQL(queryENUM.READ_ALL) = "SELECT EducationCode, EducationName FROM Tbl_Education"
        _SQL(queryENUM.READ) = "SELECT EducationCode, EducationName FROM Tbl_Education WHERE EducationCode = @Code"
        _SQL(queryENUM.INSERT) = "INSERT INTO Tbl_Education ( EducationCode, EducationName ) VALUES ( @Code, @Name )"
        _SQL(queryENUM.UPDATE) = "UPDATE Tbl_Education SET EducationName = @Name WHERE EducationCode = @Code"
        _SQL(queryENUM.DELETE) = "DELETE FROM Tbl_Education WHERE EducationCode = @Code"
    ...
    Private Sub EducationCodeForm_Closing(sender As Object, e As CancelEventArgs) Handles Me.Closing
        checkChangedField()
    End Sub

    Private Sub EducationCodeForm_Activated(sender As Object, e As EventArgs) Handles Me.Activated
    ...
    Private Sub control_Changed(sender As Object, e As EventArgs) Handles txtName.TextChanged, txtCode.TextChanged
    ...
    Private Sub control_KeyDown(sender As Object, e As KeyEventArgs) Handles txtName.KeyDown, txtCode.KeyDown
        If e.KeyCode = Keys.Enter Then
            e.SuppressKeyPress = True   'This will prevent ding sound.
            If sender.Name = "txtCode" Then
                txtName.Focus()
            ElseIf sender.Name = "txtName" Then
                menuTS.Focus()
            End If
        End If
    End Sub
    ...
    Private Sub fillGrid()
        ...
        If reader.HasRows Then
            While reader.Read()
                grdNavi.Rows.Add(reader("EducationCode"), reader("EducationName"))
            End While
            reader.Close()
    ...
    Private Sub fillData(ByVal code As String)
        ...
        If reader.HasRows Then
            reader.Read()
            With reader
                txtCode.Text = .Item("EducationCode")
                txtCode.ReadOnly = True
                txtName.Text = .Item("EducationName")
            End With
        End If
